...
-- Update the calculate_average_auction_results function to only allocate items to bidders who bid at or above the average price
...
BEGIN
    ...
    LOOP
        ...
        IF total_winning_quantity > 0 THEN
            ...
            -- Second pass: create auction results with average pricing, but only for bids at or above average price
            FOR bid_record IN
                SELECT b.*, (b.bid_amount / b.quantity_requested) as price_per_unit
                FROM public.bids b
                JOIN public.items i ON i.id = b.item_id
            ...
            LOOP
                ...
                -- Only allocate to bidders whose price per unit is at or above the average price
                IF bid_record.price_per_unit >= average_price THEN
                    DECLARE
                        quantity_to_allocate INTEGER;
